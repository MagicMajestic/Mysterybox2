...
class GiveawayButton(discord.ui.View):
    def __init__(self, giveaway_id=None, timeout=None):
    ...
    async def participate_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        logger.info(f"Initial giveaway_id: {giveaway_id}")

        if not giveaway_id and interaction.message and interaction.message.embeds:
            embed = interaction.message.embeds[0]
            if embed.footer and embed.footer.text:
                # Extract ID from footer text like "ID розыгрыша: 123456-123456789"
                footer_text = embed.footer.text
                logger.info(f"Footer text: {footer_text}")
                if "ID розыгрыша:" in footer_text:
                    giveaway_id = footer_text.split("ID розыгрыша:")[-1].strip()
                    logger.info(f"Extracted giveaway_id from footer: {giveaway_id}")

        if not giveaway_id:
            logger.error("Could not determine giveaway_id")
            await interaction.response.send_message("Не удалось определить ID розыгрыша.", ephemeral=True)
            return

        logger.info(f"Final giveaway_id: {giveaway_id}")

        # Get the cog instance
        giveaway_cog = interaction.client.get_cog("GiveawayCog")
        logger.info(f"Giveaway cog found: {giveaway_cog is not None}")

        if giveaway_cog:
            await giveaway_cog.add_participant(interaction, giveaway_id)
        else:
            logger.error("GiveawayCog not found")
...
class GiveawayCog(commands.Cog):
    def __init__(self, bot):
        ...
        # Флаг для отслеживания готовности бота
        self.bot_ready = False

    async def on_bot_ready(self):
        """Called when bot is fully ready - restore giveaways and UI Views"""
    ...
    def reload_active_giveaways(self):
        """Restart timers and restore UI Views for any active giveaways when bot starts/restarts"""
    ...
    async def restore_giveaway_view(self, giveaway_id, giveaway):
        """Restore the UI View (button) for an active giveaway after bot restart"""
        try:
            ...
            if not channel_id or not message_id:
                logger.warning(f"Missing channel_id or message_id for giveaway {giveaway_id}")
                return
            ...
            logger.info(f"Restored UI View for giveaway {giveaway_id}")

        except Exception as e:
            logger.error(f"Error restoring view for giveaway {giveaway_id}: {e}")
            logger.error(traceback.format_exc())
    ...
    async def end_giveaway(self, giveaway_id):
        ...
        try:
            ...
            if participants:
                ...
                # Select random prizes
                prize_list = []
                for _ in winners:
                    if giveaway.get("assigned_prizes"):
                        prize_list.append(random.choice(list(giveaway["assigned_prizes"].values())))
                    elif self.prizes:
                        prize_list.append(random.choice(list(self.prizes.values())))
                    else:
                        prize_list.append("Mystery Prize")  # Default if no prizes available

                winner_details = "\n".join(
                    [f"**{mention}** выиграл **{prize}**" for mention, prize in zip(winner_mentions, prize_list)]
                )

                # Send the winner announcement
                embed = discord.Embed(
                    title="🎉 Розыгрыш завершен! 🎉",
                    description=winner_details,
                ...
                if gif_id and gif_id in self.gifs:
                    ...
                    if gif_path and os.path.exists(gif_path):
                        ...
                        await channel.send(
                            content=f"Поздравляем {','.join(winner_mentions)}!",
                            embed=embed,
                            file=file
                        )
                    else:
                ...
                if message:
                    ...
                    if original_embed:
                        ...
                        original_embed.description = f"**Победители:**\n\n{winner_details}"
    ...
    @app_commands.describe(
        ...
        num_winners="Количество победителей (по умолчанию 1)"
    )
    ...
    @app_commands.command(name="setwinners", description="Изменить количество победителей в розыгрыше")
    @app_commands.describe(
        giveaway_id="ID розыгрыша",
        num_winners="Новое количество победителей"
    )
    @app_commands.default_permissions(administrator=True)
    async def set_winners(self, interaction: discord.Interaction, giveaway_id: str, num_winners: int):
        # Проверяем разрешения и права администратора
        if not await self.is_admin(interaction):
            return
        ...
        if giveaway.get("ended", False):
            await interaction.response.send_message("Невозможно изменить количество победителей в завершенном розыгрыше.", ephemeral=True)
            return
        ...
        giveaway["num_winners"] = num_winners
        save_giveaways(self.giveaways)

        await interaction.response.send_message(
            f"Количество победителей для розыгрыша **{giveaway['title']}** изменено на **{num_winners}**.",
            ephemeral=True
        )
        logger.info(f"Updated number of winners for giveaway {giveaway_id} to {num_winners}")
